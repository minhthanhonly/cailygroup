...
//  import IntlMessages from './language-provider/IntlMessages';
 ...
 export default class DynamicOptionList extends React.Component {
   ...
   editOption(option_index, e) {
     ...
     const val = (this_element.custom_options[option_index].value !== this._setValue(this_element.custom_options[option_index].text)) ? this_element.custom_options[option_index].value : this._setValue(e.target.value);

     this_element.custom_options[option_index].text = e.target.value;
     this_element.custom_options[option_index].value = val;
     this.setState({
   ...
   editValue(option_index, e) {
     const this_element = this.state.element;
     const val = (e.target.value === '') ? this._setValue(this_element.custom_options[option_index].text) : e.target.value;
     this_element.custom_options[option_index].value = val;
     this.setState({
       element: this_element,
       dirty: true,
   ...
   addOption(index) {
     const this_element = this.state.element;
     this_element.custom_options.splice(index + 1, 0, { value: '', text: '', key: ID.uuid() });
     this.props.updateElement.call(this.props.preview, this_element);
   }

   removeOption(index) {
     const this_element = this.state.element;
     this_element.custom_options.splice(index, 1);
     this.props.updateElement.call(this.props.preview, this_element);
   }
     ...
     return (
       <div className="dynamic-option-list">
         <ul>
           ...
           {
             this.props.element.custom_options.map((option, index) => {
