  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const opentimeObject = data.find(
          (item: { config_key: string }) => item.config_key === 'opentime',
        );
        // Tìm đối tượng có config_key là 'closetime'
        const closetimeObject = data.find(
          (item: { config_key: string }) => item.config_key === 'closetime',
        );
        const lunchStarttimeObject = data.find(
          (item: { config_key: string }) => item.config_key === 'openlunch',
        );
        const lunchEndtimeObject = data.find(
          (item: { config_key: string }) => item.config_key === 'closelunch',
        );

        if (
          opentimeObject &&
          closetimeObject &&
          lunchStarttimeObject &&
          lunchEndtimeObject
        ) {
          ...
          const lunchStartParts = lunchStarttimeObject.config_value
            .trim()
            .split(':');
          const lunchEndParts = lunchEndtimeObject.config_value
            .trim()
            .split(':');

          // Chắc chắn rằng có đủ phần tử để tránh lỗi
          if (
            opentimeParts.length === 2 &&
            closetimeParts.length === 2 &&
            lunchStartParts.length === 2 &&
            lunchEndParts.length === 2
          ) {
            ...
            if (
              ...
              !isNaN(closeHour) &&
              !isNaN(closeMinute) &&
              !isNaN(lunchHourStart) &&
              !isNaN(lunchMinuteStart) &&
              !isNaN(lunchHourEnd) &&
              !isNaN(lunchMinuteEnd)
            ) {
          ...
          console.error(
            'Không tìm thấy đối tượng opentime hoặc closetime trong dữ liệu',
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchTimeValues = async () => {
      try {
        ...
        setTimeLunchStartMinutes(
          parseInt(timeValues.lunchStartMinutes, 10) || 0,
        );
    ...
    try {
      if (isTimeChangedInput === true) {
        ...
        const dataUpdateArray = [
          { id: 1, config_key: 'opentime', hoursMinutes: timeInputString },
        ];
  ...
  const handleSaveOutTime = async () => {
    try {
      if (isTimeChangedOutput === true) {
        ...
        const dataUpdateArray = [
          { id: 2, config_key: 'closetime', hoursMinutes: timeOutString },
        ];
        // Thêm các đối tượng khác nếu cần
  ...
  const handleSaveLunchStartTime = async () => {
    try {
      if (isTimeLunchStart === true) {
        const formattedLunchStartHouse = String(timeLunchStartHouse).padStart(
          2,
          '0',
        );
        const formattedLunchStartMinutes = String(
          timeLunchStartMinutes,
        ).padStart(2, '0');
        // Xác nhận giá trị trước khi gửi request
        const timeOutString = `${formattedLunchStartHouse}:${formattedLunchStartMinutes}`;
        const dataUpdateArray = [
          { id: 2, config_key: 'openlunch', hoursMinutes: timeOutString },
        ];
        // Thêm các đối tượng khác nếu cần
        const response = await axios.post(
          'timecards/getInput',
        ...
        if (response.status === 200) {
          toast.success(
            `Chỉnh giờ bắt đầu nghỉ trưa là: ${timeOutString} thành công`,
          );
          setTimeout(() => {
  ...
  const handleSaveLunchEndTime = async () => {
    try {
      if (isTimeLunchEnd === true) {
        const formattedLunchEndHouse = String(timeLunchEndHouse).padStart(
          2,
          '0',
        );
        const formattedLunchEndMinutes = String(timeLunchEndMinutes).padStart(
          2,
          '0',
        );
        // Xác nhận giá trị trước khi gửi request
        ...
        const dataUpdateArray = [
          { id: 2, config_key: 'closelunch', hoursMinutes: timeOutString },
        ];
        // Thêm các đối tượng khác nếu cần
        const response = await axios.post(
          'timecards/getInput',
        ...
        if (response.status === 200) {
          toast.success(
            `Chỉnh giờ kết thúc nghỉ trưa là: ${timeOutString} thành công`,
          );
          setTimeout(() => {
    ...
    <>
      ...
      <div className="center">
        ...
        {serverMessage == '' ? (
          ...
          <div className="box-bg">
            <p className="bg bg-green">{serverMessage}</p>
          </div>
        )}
      </div>
      <div className="card-box">
        <div className="card-box--center">
          ...
          />
          <button
            className="btn btn--widthAuto"
            onClick={handleSaveTimeInput}
            disabled={!isTimeChangedInput}
          >
            {' '}
            Cập nhật
          </button>
        ...
        <div className="card-box--center">
          ...
          <button
            className="btn btn--widthAuto"
            onClick={handleSaveLunchStartTime}
            disabled={!isTimeLunchStart}
          >
            {' '}
            Cập nhật{' '}
          </button>
        ...
        <div className="card-box--center">
          ...
          <button
            className="btn btn--widthAuto"
            onClick={handleSaveLunchEndTime}
            disabled={!isTimeLunchEnd}
          >
            {' '}
            Cập nhật{' '}
          </button>
